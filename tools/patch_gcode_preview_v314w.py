...
s = s.replace('<div class="app-version" id="app-version">V3.14v</div>', '<div class="app-version" id="app-version">V3.14w</div>', 1)
if '<div class="app-version" id="app-version">V3.14w</div>' not in s:
    s, _ = re.subn(r'(<div\b[^>]*class=["\'][^"\']*\bapp-version\b[^"\']*["\'][^>]*>\s*)V3\.14[a-z](\s*</div>)', r'\1V3.14w\2', s, count=1, flags=re.I)
...
for oldver in ('V3.14s','V3.14v'):
    s = s.replace(
        f"    const header=`; 20-20-TOOLBOX {oldver}\\n; slicer_engine = LOCAL ${{ver}}",
