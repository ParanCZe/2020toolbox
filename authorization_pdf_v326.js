...
(() => {
  ...
  const INSTALLER_URL = 'https://raw.githubusercontent.com/ParanCZe/2020toolbox/cce49b9a5acb95c880fa76c1df9a381647ae3dfb/AuthorizationBridge/INSTALL_AND_START.bat';
  ...
  window.signAuthorizationBatch = async function() {
    ...
    if (!testMode && (!authVersionAtLeast(bridge.version, '2.1.6') || !bridge.features?.tsa_preflight || !bridge.features?.local_sign_approval || !bridge.features?.docmdp_annotate)) {
      toast('Kvůli bezpečnému podpisu a správnému DocMDP omezení aktualizuj AuthorizationBridge na 2.1.6.', true);
      return;
    }
