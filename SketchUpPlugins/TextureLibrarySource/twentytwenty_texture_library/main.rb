...
module TwentyTwenty
  module TextureLibrary
    ...
    PREF_KEY='twentytwenty_texture_library_v018'.freeze
    ...
    def download_https(url,target,limit=5)
      ...
      req=Net::HTTP::Get.new(uri.request_uri,{'User-Agent'=>'20-20 Texture Library/0.1.8'})
...
function resetAll(){xr.value=xn.value=0;yr.value=yn.value=0;rr.value=rn.value=0;sketchup.reset_texture()}
    ...
    def add_texture_club_texture(key)
      category=TEXTURE_CLUB_CATEGORIES.find{|x|x['key'].to_s==key.to_s}
      return UI.messagebox('Neznámá kategorie Texture Club.') unless category
