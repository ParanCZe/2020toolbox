...
module TwentyTwenty
  module TextureLibrary
    ...
    PREF_KEY='twentytwenty_texture_library_v021'.freeze
    ...
    POLYHAVEN_UA='20-20 Texture Library/0.2.1 (SketchUp; Poly Haven API)'.freeze
    ...
    def download_https(url,target,limit=5)
      ...
      req=Net::HTTP::Get.new(uri.request_uri,{'User-Agent'=>'20-20 Texture Library/0.2.1'})
    ...
    # RAKO product pages expose a dedicated zoom image that is substantially
    # cleaner than the small product-card image used by SIKO. For Rave we
    # prefer that official manufacturer image and keep SIKO as a fallback.
    def rako_rave_highres_url(rec)
      code=rako_product_code(rec)
      raise 'Chybí kód RAKO produktu.' if code.empty?
      page_url="https://www.rako.cz/cs/#{code.downcase}"
      uri=URI.parse(page_url)
      req=Net::HTTP::Get.new(uri.request_uri,{'User-Agent'=>'20-20 Texture Library/0.2.1'})
      response=Net::HTTP.start(uri.host,uri.port,use_ssl:true,open_timeout:8,read_timeout:20){|http|http.request(req)}
      raise "RAKO produkt HTTP #{response.code}" unless response.is_a?(Net::HTTPSuccess)
      body=response.body.to_s
      match=body.match(/href=["']([^"']*\/eshop-product-detail-zoom\.jpg(?:\?[^"']*)?)["']/i)
      raise 'RAKO high-res obrázek nebyl nalezen.' unless match
      URI.join(page_url,match[1]).to_s
    end
    ...
    def siko_texture_path(rec)
      ...
      if rako_rave_record?(rec)
        begin
          hq_url=rako_rave_highres_url(rec)
          hq_target=File.join(SIKO_CACHE,"#{safe}_rako_hq.jpg")
          download_https(hq_url,hq_target) unless File.file?(hq_target) && File.size(hq_target)>1024
          return hq_target if File.file?(hq_target) && File.size(hq_target)>1024
        rescue StandardError
          # Manufacturer source unavailable -> silently keep the proven SIKO fallback.
        end
      end
