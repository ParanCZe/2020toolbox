...
module TwentyTwenty
  module TextureLibrary
    ...
    PREF_KEY='twentytwenty_texture_library_v023'.freeze
    ...
    SIKO_RAVE_HQ_CACHE=File.join(LIB_ROOT,'siko_rave_hq_v3').freeze
    ...
    POLYHAVEN_UA='20-20 Texture Library/0.2.3 (SketchUp; Poly Haven API)'.freeze
    ...
    def download_https(url,target,limit=5)
      ...
      req=Net::HTTP::Get.new(uri.request_uri,{'User-Agent'=>'20-20 Texture Library/0.2.3'})
    ...
    def siko_product_highres_url(rec)
      ...
      req=Net::HTTP::Get.new(uri.request_uri,{
        'User-Agent'=>'Mozilla/5.0 (Windows NT 10.0; Win64; x64) 20-20 Texture Library/0.2.3',
      ...
      # SIKO catalog snapshot currently stores 96×96 thumbnails. The actual
      # product page serves a 515×515 main product image (plus smaller 300/96/65).
    ...
    def siko_rave_highres_path(rec)
      ...
      target=File.join(SIKO_RAVE_HQ_CACHE,"#{safe}_siko_main.jpg")
    ...
    def siko_texture_path(rec)
      if rako_rave_record?(rec)
        begin
          # Prefer exactly the sharp main image shown on the SIKO product page.
          # The catalog JSON contains only a 96×96 thumbnail, which caused the
          # visibly blurred material in SketchUp.
