...
module TwentyTwenty
  module TextureLibrary
    ...
    PREF_KEY='twentytwenty_texture_library_v024'.freeze
    ...
    RAKO_RAVE_CACHE=File.join(LIB_ROOT,'rako_rave_v2').freeze
    SIKO_RAVE_HQ_CACHE=File.join(LIB_ROOT,'siko_rave_hq_v4').freeze
    RAKO_RAVE_ZIP=File.join(RAKO_RAVE_CACHE,'rako-rave.zip').freeze
    RAKO_RAVE_EXTRACTED=File.join(RAKO_RAVE_CACHE,'extracted').freeze
    RAKO_RAVE_ZIP_URL='https://www.rako.cz/file/edee/produkty-serie/rako-rave.zip'.freeze
    SIKO_RAVE_1200_URLS={
      'DAK62900.1'=>'https://api.siko.cz/medias/im-1200-DAK62900.1-004.jpg?context=bWFzdGVyfGF6dXJlaW1hZ2VzfDQzNjUzNHxpbWFnZS9qcGVnfGFEYzFMMmd5T0M4eE5EWTVNek00TWpVeU1EZzJNaTlwYlMweE1qQXdYMFJCU3pZeU9UQXdMakZmTURBMExtcHdad3xlNDE3YWM3ZTVlMmVlNTBmNTViZmIyNzJhODI3ZDViMDgyMWZlMTFlZjQxM2Y2MmU1NGZmMGI5M2QyOTY2OTQx',
      'DAK62899.1'=>'https://api.siko.cz/medias/im-1200-DAK62899.1.jpg?context=bWFzdGVyfGF6dXJlaW1hZ2VzfDMyMDUyM3xpbWFnZS9qcGVnfGFEaGxMMmd6TWk4eE5EWTVNek0zT0RNNU1qQTVOQzlwYlMweE1qQXdYMFJCU3pZeU9EazVMakV1YW5CbnwxZWEwZTRkYmM0N2YxYWI4NGVlOGE5ZjczZmVjM2YzMTk4MThkNTNhYWRjMDAxMjlkMWI4ODA3ZjM4Y2ZjMjAx',
      'DAK62901.1'=>'https://api.siko.cz/medias/im-1200-DAK62901.1.jpg?context=bWFzdGVyfGF6dXJlaW1hZ2VzfDQwMTQzMXxpbWFnZS9qcGVnfGFEUXhMMmhpTWk4eE5EWTVNelF4TmpZMk5URXhPQzlwYlMweE1qQXdYMFJCU3pZeU9UQXhMakV1YW5Cbnw5NzllM2NlMjA3YTk1NzE1MzhiMjRhOTdhYjVmYzk4ZWNlZjVjYTI0OWVmNGFlZTBkZGQ2NjUxYjc0YjRlN2Jl'
    }.freeze
    ...
    POLYHAVEN_UA='20-20 Texture Library/0.2.4 (SketchUp; Poly Haven API)'.freeze
    ...
    def ensure_library!
      FileUtils.mkdir_p([TEX_ROOT,SIKO_CACHE,EGGER_CACHE,POLYHAVEN_CACHE,RAKO_RAVE_CACHE,SIKO_RAVE_HQ_CACHE])
    ...
    def read_siko_catalog(kind='concrete')
      ...
      items.each do |rec|
        direct=SIKO_RAVE_1200_URLS[rec['code'].to_s]
        rec['image_url']=direct unless direct.to_s.empty?
      end
      items
    rescue StandardError
      []
    ...
    def download_https(url,target,limit=5)
      ...
      req=Net::HTTP::Get.new(uri.request_uri,{'User-Agent'=>'20-20 Texture Library/0.2.4'})
    ...
    def rako_rave_record?(rec)
      text=[
        rec['name'],rec['filter_brand'],rec['brand'],
        rec['filter_product'],rec['series'],rec['category_name']
      ].compact.join(' ').downcase
      text.include?('rako') && text.include?('rave')
    end

    def rako_product_code(rec)
      code=rec['code'].to_s.strip
      code=code.sub(/\.\d+\z/,'')
      code.gsub(/[^0-9A-Za-z_-]+/,'')
    end

    def ensure_rako_rave_archive
      FileUtils.mkdir_p(RAKO_RAVE_CACHE)
      unless File.file?(RAKO_RAVE_ZIP) && File.size(RAKO_RAVE_ZIP)>1_000_000
        download_https(RAKO_RAVE_ZIP_URL,RAKO_RAVE_ZIP)
      end

      marker=File.join(RAKO_RAVE_EXTRACTED,'.20-20-extracted')
      unless File.file?(marker)
        FileUtils.rm_rf(RAKO_RAVE_EXTRACTED)
        FileUtils.mkdir_p(RAKO_RAVE_EXTRACTED)

        ok=false
        if Sketchup.platform==:platform_win
          begin
            ok=system('tar.exe','-xf',RAKO_RAVE_ZIP,'-C',RAKO_RAVE_EXTRACTED)
          rescue StandardError
            ok=false
          end
          unless ok
            zip=RAKO_RAVE_ZIP.gsub("'","''")
            dst=RAKO_RAVE_EXTRACTED.gsub("'","''")
            cmd="Expand-Archive -LiteralPath '#{zip}' -DestinationPath '#{dst}' -Force"
            ok=system('powershell.exe','-NoProfile','-ExecutionPolicy','Bypass','-Command',cmd)
          end
        else
          ok=system('unzip','-o',RAKO_RAVE_ZIP,'-d',RAKO_RAVE_EXTRACTED)
        end
        raise 'Oficiální RAKO Rave ZIP se nepodařilo rozbalit.' unless ok
        File.write(marker,Time.now.to_i.to_s)
      end
      RAKO_RAVE_EXTRACTED
    end

    def siko_product_highres_url(rec)
      page_url=rec['product_url'].to_s
      raise 'U produktu chybí SIKO odkaz.' if page_url.empty?
      code=rec['code'].to_s.strip
      raise 'U produktu chybí SIKO kód.' if code.empty?

      # Known RAKO Rave 60x60 variants use the direct 1200 px SIKO master.
      direct=SIKO_RAVE_1200_URLS[code]
      return direct unless direct.to_s.empty?

      uri=URI.parse(page_url)
      req=Net::HTTP::Get.new(uri.request_uri,{
        'User-Agent'=>'Mozilla/5.0 (Windows NT 10.0; Win64; x64) 20-20 Texture Library/0.2.4',
        'Accept'=>'text/html,application/xhtml+xml'
      })
      response=Net::HTTP.start(uri.host,uri.port,use_ssl:true,open_timeout:8,read_timeout:25){|http|http.request(req)}
      raise "SIKO produkt HTTP #{response.code}" unless response.is_a?(Net::HTTPSuccess)

      body=response.body.to_s.gsub('\\u0026','&').gsub('&amp;','&')
      code_re=Regexp.escape(code)
      matches=body.scan(%r{(https://api\.siko\.cz/medias/im-([0-9]+)-#{code_re}\.jpg\?context=[^"'<>\\s]+)}i)
      raise 'Na stránce SIKO nebyla nalezena produktová fotografie.' if matches.empty?

      # Other RAKO Rave products fall back to the largest image exposed by the SIKO product page.
      best=matches.max_by{|pair|pair[1].to_i}
      best[0]
    end

    def siko_rave_highres_path(rec)
      safe=rec['code'].to_s.gsub(/[^0-9A-Za-z._-]+/,'_')
      target=File.join(SIKO_RAVE_HQ_CACHE,"#{safe}_siko_1200.jpg")
      return target if File.file?(target) && File.size(target)>20_000

      url=siko_product_highres_url(rec)
      download_https(url,target)
      raise 'SIKO HQ obrázek je podezřele malý.' unless File.file?(target) && File.size(target)>20_000
      target
    end

    def rako_rave_texture_path(rec)
      code=rako_product_code(rec)
      raise 'Chybí kód RAKO produktu.' if code.empty?
      root=ensure_rako_rave_archive

      candidates=Dir.glob(File.join(root,'**',"#{code}_*.{jpg,jpeg,png}"),File::FNM_CASEFOLD)
      raise "V oficiálním RAKO Rave balíku chybí #{code}." if candidates.empty?

      # _1 is the manufacturer's first full-resolution design variant. All Rave
      # square tiles in the official archive are 1200 px, which is higher and
      # substantially less compressed than RAKO's 990 px web zoom.
      source=candidates.find{|p|File.basename(p).downcase=="#{code.downcase}_1.jpg"} || candidates.first
      safe=rec['code'].to_s.gsub(/[^0-9A-Za-z._-]+/,'_')
      target=File.join(RAKO_RAVE_CACHE,"#{safe}_official_1200#{File.extname(source).downcase}")
      FileUtils.cp(source,target) unless File.file?(target) && File.size(target)==File.size(source)
      target
    end

    def siko_texture_path(rec)
      if rako_rave_record?(rec)
        begin
          # Prefer the direct 1200 px SIKO master for known RAKO Rave variants.
          # Other Rave products fall back to the best product-page image.
          return siko_rave_highres_path(rec)
        rescue StandardError
          begin
            return rako_rave_texture_path(rec)
          rescue StandardError
            # Last-resort fallback below keeps painting functional.
          end
        end
      end
