...
module TwentyTwenty
  module TextureLibrary
    ...
    PREF_KEY='twentytwenty_texture_library_v022'.freeze
    ...
    POLYHAVEN_UA='20-20 Texture Library/0.2.2 (SketchUp; Poly Haven API)'.freeze
    ...
    def ensure_library!
      FileUtils.mkdir_p([TEX_ROOT,SIKO_CACHE,EGGER_CACHE,POLYHAVEN_CACHE,RAKO_RAVE_CACHE])
    ...
    def download_https(url,target,limit=5)
      ...
      req=Net::HTTP::Get.new(uri.request_uri,{'User-Agent'=>'20-20 Texture Library/0.2.2'})
    ...
    def siko_texture_path(rec)
      if rako_rave_record?(rec)
        begin
          ...
          # If the official manufacturer pack cannot be reached/extracted, keep
          # the normal SIKO fallback so painting never becomes unusable.
        end
      end
