...
module TwentyTwenty
  module TextureLibrary
    ...
    PREF_KEY='twentytwenty_texture_library_v014'.freeze
    ...
    def download_https(url,target,limit=5)
      ...
      req=Net::HTTP::Get.new(uri.request_uri,{'User-Agent'=>'20-20 Texture Library/0.1.4'})
    ...
    # SIKO sometimes supplies a square material swatch for a rectangular tile.
    # Forcing a square image into e.g. 60x120 cm visibly stretches the pattern.
    # Use the real tile dimensions when the image aspect agrees; otherwise preserve
    # the source image aspect while keeping approximately the same real-world area.
    def siko_texture_size(rec,path)
      ...
      begin
        ...
        iw=rep.width.to_f
        ih=rep.height.to_f
        return [w.mm,h.mm,'product'] unless iw>0 && ih>0
        ...
        if [normal_error,swapped_error].min<=0.18
          swapped_error<normal_error ? [h.mm,w.mm,'product_rotated'] : [w.mm,h.mm,'product']
        else
          area=w*h
          fixed_w=Math.sqrt(area*image_ratio)
          fixed_h=area/fixed_w
          [fixed_w.mm,fixed_h.mm,'image_aspect_preserved']
        end
      rescue StandardError
        [w.mm,h.mm,'product']
      end
