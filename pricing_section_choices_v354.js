(()=>{
  ...
  function addSection(type){
    ...
    }else items=(def.items||[]).map(name=>({id:uid('i'),on:true,name,description:'',qty:0,price:0}));
  ...
  function installSectionButton(){
    ...
    menu.querySelectorAll('[data-section]').forEach(b=>b.onclick=e=>{e.stopPropagation();menu.hidden=true;addSection(b.dataset.section)});
  ...
  function apply(){style();installSectionButton();installRowChoices()}
