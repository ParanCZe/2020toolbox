(()=>{
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      .crop334{position:absolute;inset:0;z-index:220;background:rgba(255,255,255,.10);cursor:crosshair;touch-action:none;user-select:none;overflow:visible}
      .crop334-toolbar{position:absolute;left:6px;top:6px;z-index:260;display:flex;align-items:center;gap:4px;padding:4px;background:#fff;border:1px solid #18181b;border-radius:7px;box-shadow:0 2px 10px rgba(0,0,0,.18);cursor:default}
      .crop334-toolbar button{height:28px;padding:0 9px;border:1px solid #d4d4d8;border-radius:5px;background:#fff;color:#18181b;font-size:10px;font-weight:700;cursor:pointer;white-space:nowrap;pointer-events:auto}
      ...
      .crop334-rect{position:absolute;border:2px solid #18181b;background:rgba(247,241,151,.12);box-shadow:0 0 0 9999px rgba(255,255,255,.45);cursor:move;box-sizing:border-box;z-index:230;pointer-events:auto;touch-action:none}
      .crop334-h{position:absolute;width:13px;height:13px;border:1px solid #18181b;background:#f7f197;border-radius:2px;box-sizing:border-box;z-index:245;pointer-events:auto;touch-action:none}
      .crop334-h.nw{left:-7px;top:-7px;cursor:nwse-resize}.crop334-h.n{left:50%;top:-7px;transform:translateX(-50%);cursor:ns-resize}.crop334-h.ne{right:-7px;top:-7px;cursor:nesw-resize}
      .crop334-h.e{right:-7px;top:50%;transform:translateY(-50%);cursor:ew-resize}.crop334-h.se{right:-7px;bottom:-7px;cursor:nwse-resize}.crop334-h.s{left:50%;bottom:-7px;transform:translateX(-50%);cursor:ns-resize}.crop334-h.sw{left:-7px;bottom:-7px;cursor:nesw-resize}.crop334-h.w{left:-7px;top:50%;transform:translateY(-50%);cursor:ew-resize}
      .crop334-point{position:absolute;width:12px;height:12px;border:1px solid #18181b;background:#f7f197;border-radius:50%;transform:translate(-50%,-50%);cursor:grab;z-index:245;pointer-events:auto;touch-action:none}
      .crop334-help{position:absolute;left:8px;bottom:8px;z-index:250;background:rgba(255,255,255,.92);border:1px solid #d4d4d8;border-radius:5px;padding:4px 7px;font-size:9px;color:#52525b;pointer-events:none}
  ...
  function openCrop(host,obj,isMood){
    ...
    let polyPts=oldMode==='polygon'&&oldPoints?oldPoints.map(p=>({...p})):[];
    let rect=rectFromPoints(obj);

    const layer=document.createElement('div');layer.className='crop334';
    layer.innerHTML=`<div class="crop334-toolbar"><button type="button" data-mode="rect">OBDÉLNÍK</button><button type="button" data-mode="polygon">POLYGON</button><span class="sep"></span><button type="button" class="ok" title="Potvrdit">✓</button><button type="button" class="cancel" title="Zrušit">×</button><button type="button" class="reset" title="Zrušit ořez">↺</button></div><svg class="crop334-svg" viewBox="0 0 100 100" preserveAspectRatio="none"><polygon fill="rgba(247,241,151,.14)" stroke="#18181b" stroke-width=".45" vector-effect="non-scaling-stroke"></polygon></svg><div class="crop334-help"></div>`;
    ...
    function clearShapeNodes(){layer.querySelectorAll('.crop334-point,.crop334-rect').forEach(n=>n.remove())}
    function redrawPolygon(){
      clearShapeNodes();poly.style.display='';poly.setAttribute('points',polyPts.map(p=>`${p.x*100},${p.y*100}`).join(' '));
      polyPts.forEach((p,i)=>{const h=document.createElement('span');h.className='crop334-point';h.style.left=(p.x*100)+'%';h.style.top=(p.y*100)+'%';h.dataset.i=i;layer.appendChild(h)});
    }
    function redrawRect(){
      clearShapeNodes();poly.style.display='none';
    ...
    function redraw(){
      ...
      help.textContent=mode==='rect'?'Táhni mimo rámeček pro nový výřez. Uvnitř výřez posouváš, body na hranách a rozích mění velikost.':'Klikáním přidávej body. Žluté body můžeš kdykoliv přetáhnout.';
    ...
    function setMode(next){
      ...
      mode=next;
      if(mode==='rect'){
        // při návratu z polygonu nabídni normální upravitelný obdélník
        if(rect.x2-rect.x1<.01||rect.y2-rect.y1<.01)rect={x1:.1,y1:.1,x2:.9,y2:.9};
      }else if(!polyPts.length&&oldMode==='polygon'&&oldPoints){polyPts=oldPoints.map(p=>({...p}))}
      redraw();
    }

    // Ovládací panel nesmí spouštět drag rodičovského média, ale události musí dojít až na tlačítka.
    ['pointerdown','mousedown','touchstart'].forEach(type=>toolbar.addEventListener(type,e=>e.stopPropagation(),type==='touchstart'?{passive:true}:false));
    toolbar.querySelector('[data-mode="rect"]').onclick=e=>{e.preventDefault();e.stopPropagation();setMode('rect')};
    toolbar.querySelector('[data-mode="polygon"]').onclick=e=>{e.preventDefault();e.stopPropagation();setMode('polygon')};
    ...
    layer.addEventListener('pointerdown',e=>{
      ...
      if(mode==='polygon'){
        ...
        polyPts.push(p);redrawPolygon();return;
      }
      const hh=e.target.closest('.crop334-h'),rr=e.target.closest('.crop334-rect');
    ...
    layer.addEventListener('pointermove',e=>{
      ...
      if(drag.type==='point'){polyPts[drag.i]=p;redrawPolygon();return}
      if(drag.type==='new'){
      ...
      if(drag.type==='move'){
        ...
        const x1=clamp(drag.base.x1+dx,0,1-w),y1=clamp(drag.base.y1+dy,0,1-h);rect={x1,y1,x2:x1+w,y2:y1+h};redrawRect();return;
      }
      if(drag.type==='resize'){
        const r={...drag.base},k=drag.handle;
    ...
    toolbar.querySelector('.ok').onclick=e=>{
      e.preventDefault();e.stopPropagation();
      if(mode==='polygon'&&polyPts.length<3){alert('Pro polygonální ořez označ alespoň 3 body.');return}
      if(mode==='rect'&&((rect.x2-rect.x1)<.005||(rect.y2-rect.y1)<.005)){alert('Vytvoř větší obdélníkový výřez.');return}
      obj.cropMode=mode;obj.cropPoints=mode==='rect'?rectPoints(rect):polyPts.map(p=>({...p}));layer.remove();apply(host,obj);save();
    };
    toolbar.querySelector('.cancel').onclick=e=>{e.preventDefault();e.stopPropagation();obj.cropMode=oldMode;obj.cropPoints=oldPoints;layer.remove();apply(host,obj)};
    toolbar.querySelector('.reset').onclick=e=>{e.preventDefault();e.stopPropagation();obj.cropMode=null;obj.cropPoints=null;layer.remove();apply(host,obj);save()};
    redraw();
  }
  ...
  function init(){css();document.addEventListener('click',intercept,true)}
