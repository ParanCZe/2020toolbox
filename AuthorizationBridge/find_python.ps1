...
function Test-PythonExe([string]$Path) {
    ...
    # Microsoft Store execution aliases in WindowsApps are only launch stubs,
    # not a usable Python runtime for venv/pip.
    if ($Path -match '\\Microsoft\\WindowsApps\\python(3)?\.exe    try {
