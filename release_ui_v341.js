(()=>{
  ...
  function showWhatsNew(){
    ...
    overlay.innerHTML=`<div id="toolbox-release-window" role="dialog" aria-modal="true" aria-labelledby="toolbox-release-title"><h2 id="toolbox-release-title">Co je nového — ${RELEASE}</h2><p>Aktualizace 20-20 TOOLBOX.</p><ul><li>Cenová nabídka byla překopána podle šablony Libertas.</li><li>Nové nabídky jsou nyní prázdné — žádné předvyplněné práce ani profese.</li><li>Přibylo nastavení projektu, objednatele, data, výchozí hodinové sazby a DPH.</li><li>Export má firemní žluté sekce, černé hlavičky, souhrn vstupů/výstupů a rekapitulaci ceny.</li><li>Tlačítko Odhad zůstává součástí řádků a využívá historická data z uložených nabídek.</li></ul><button type="button" id="toolbox-release-ok">Rozumím</button></div>`;
