...
$logPath = Join-Path $helperDir 'last_install.log'
New-Item -ItemType Directory -Force -Path $helperDir | Out-Null

function Log([string]$Text) {
    try { Add-Content -Path $logPath -Value ((Get-Date -Format s) + '  ' + $Text) -Encoding UTF8 } catch {}
}

function Fail([string]$Message) {
    Log ('ERROR: ' + $Message + ' | URL=' + $ProtocolUrl)
...
try {
    Log ('START URL=' + $ProtocolUrl)

    if ([string]::IsNullOrWhiteSpace($ProtocolUrl)) {
        Fail 'Instalacni odkaz je prazdny.'
    }

    # Robustne najdi libovolny nazev *.rbz kdekoliv v URL.
    $decoded = [Uri]::UnescapeDataString($ProtocolUrl)
    $fileMatch = [regex]::Match($decoded, '(?i)([A-Za-z0-9._-]+\.rbz)')
    if (-not $fileMatch.Success) {
        Fail ('V instalacnim odkazu nebyl nalezen RBZ soubor: ' + $decoded)
    }
    ...
    if (-not (Test-Path $sketchupRoot)) {
        Fail 'Nebyla nalezena slozka SketchUp v AppData.'
    }
    ...
    # RBZ jsou embedded primo v Toolbox index.html.
    $indexUrl = 'https://raw.githubusercontent.com/ParanCZe/2020toolbox/main/index.html'
    $indexText = (Invoke-WebRequest -UseBasicParsing -Uri $indexUrl -Headers @{ 'Cache-Control'='no-cache' }).Content
