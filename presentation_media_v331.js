(()=>{
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id='presentation-media-v331-style';s.textContent=`
      ...
      .presentation-media-item img{width:100%;height:100%;display:block;object-fit:fill;pointer-events:none}
  ...
  async function onFile(e){
    ...
    const data=await fileData(f),im=await imgInfo(data);await addMedia(data,im.w,im.h,f.name);
  }
  ...
  async function handlePdf(file){
    ...
    if(pdf.numPages===1){const data=await renderPdfPage(pdf,1,1800);const inf=await imgInfo(data);await addMedia(data,inf.w,inf.h,`${file.name} · strana 1`);return}
    modal();STATE.pendingPdf={pdf,file};const grid=document.getElementById('presentation-media-pages');grid.innerHTML='';document.getElementById('presentation-media-modal').style.display='flex';
  ...
  function render(){
    ...
    items.forEach(o=>{if(!o.dataUrl)return;const d=document.createElement('div');d.className='presentation-media-item';d.dataset.id=o.id;d.style.left=(g.left+o.x*g.w)+'px';d.style.top=(g.top+o.y*g.h)+'px';d.style.width=(o.w*g.w)+'px';d.style.height=(o.h*g.h)+'px';d.innerHTML=`<img alt=""><button class="pm-delete" title="Smazat">×</button><span class="pm-resize" title="Změnit velikost"></span>`;d.querySelector('img').src=o.dataUrl;g.par.appendChild(d);wire(d,o,g)});
  }
  function wire(el,o,g){
    ...
    el.querySelector('.pm-resize').onpointerdown=e=>{e.stopPropagation();select(el);const sx=e.clientX,sy=e.clientY,ow=o.w,oh=o.h,ratio=ow/oh;e.target.setPointerCapture(e.pointerId);e.target.onpointermove=ev=>{let nw=Math.max(.03,ow+(ev.clientX-sx)/g.w),nh=nw/ratio;if(ev.shiftKey){nh=Math.max(.03,oh+(ev.clientY-sy)/g.h)}o.w=Math.min(1-o.x,nw);o.h=Math.min(1-o.y,nh);el.style.width=(o.w*g.w)+'px';el.style.height=(o.h*g.h)+'px'};e.target.onpointerup=()=>{e.target.onpointermove=null;persist()}};
  ...
  async function hydrate(){for(const list of Object.values(STATE.byPage))for(const o of list||[])if(!o.dataUrl&&o.imageRef&&typeof window.loadImageFromProject==='function'){try{o.dataUrl=await window.loadImageFromProject(o.imageRef)}catch(e){}}render()}
  ...
  function wrapStructure(){
    ...
    window.buildPageRecords=function(){const r=build.apply(this,arguments);r.forEach((rec,i)=>{const list=STATE.byPage[i+1];if(list?.length)rec.presentationMedia=list.map(o=>({...o,dataUrl:null}))});return r};
